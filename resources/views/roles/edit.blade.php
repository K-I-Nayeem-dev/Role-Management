                ...
                <div class="p-6 text-gray-900 dark:text-gray-100 ">
                    <form method="POST" action="{{ route('roles.update', $role->id) }}">
                        ...
                        <div>
                            ...
                            <div class='grid grid-cols-4'>
                                ...
                                @if ($permissions->isNotEmpty())
                                    @foreach ($permissions as $permission)
                                        <div class="mt-3">
                                            <input {{ $hasPermissions->contains($permission->name) ? 'checked' : '' }} type="checkbox" id="{{ $permission->id }}" name="permission[]" class="rounded" value="{{ $permission->name }}">
                            ...
                            <button class="bg-slate-700 py-2 rounded shadow-lg px-4 mt-3 text-sm">Update</button>
                        </div>
                    </form>
                </div>
            </div>
